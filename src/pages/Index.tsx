...
const Index = () => {
  ...
  useEffect(() => {
    ...
    document.addEventListener('webhook:triggered', handleWebhookEvent as EventListener);
    return () => {
      document.removeEventListener('webhook:triggered', handleWebhookEvent as EventListener);
    };
  }, [showNotification]);
  return <div className="min-h-screen cosmic-bg flex flex-col">
      {/* Loading screen */}
      ...
      <WebhookNotification isVisible={isNotificationVisible} message={notificationMessage} onClose={hideNotification} />
        ...
        <main className="flex-1 flex flex-col md:flex-row p-4 md:p-6 gap-6 container relative z-10">
          {session ? <>
              ...
              <div className={`w-full ${!isMobile ? 'md:w-1/2' : ''} bg-cosmos-cosmic bg-opacity-50 rounded-lg neon-border overflow-hidden flex flex-col ${isMobile ? 'h-auto' : 'h-[calc(100vh-11rem)]'} backdrop-blur-sm`}>
                <div className="p-6 flex-1 overflow-y-auto py-[22px]">
                  <FileUpload />
                </div>
              </div>
              ...
              {!isMobile && <div className="w-full md:w-1/2 bg-cosmos-cosmic bg-opacity-50 rounded-lg neon-border overflow-hidden flex flex-col h-[calc(100vh-11rem)] backdrop-blur-sm">
